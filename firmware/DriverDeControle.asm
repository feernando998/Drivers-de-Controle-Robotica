;  Assembly code generated by mikroVirtualMachine - V. 5.0.0.3
;  Date/Time: 09/09/2020 21:38:54
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$28D7			GOTO	_main
$0108	$	_mul_32x32_u:
$0108	$1303			BCF	STATUS, RP1
$0109	$1283			BCF	STATUS, RP0
;math.ppas,1570 :: 		
;math.ppas,1578 :: 		
$010A	$3022			MOVLW	34
;math.ppas,1579 :: 		
$010B	$00FC			MOVWF	STACK_12
;math.ppas,1580 :: 		
$010C	$01F8			CLRF	STACK_8
;math.ppas,1581 :: 		
$010D	$01F9			CLRF	STACK_9
;math.ppas,1582 :: 		
$010E	$01FA			CLRF	STACK_10
;math.ppas,1584 :: 		
$010F	$01FB			CLRF	STACK_11
;math.ppas,1588 :: 		
$0110	$	_NEXT:
;math.ppas,1589 :: 		
$0110	$03FC			DECF	STACK_12, F
;math.ppas,1590 :: 		
$0111	$1903			BTFSC	STATUS, Z
;math.ppas,1591 :: 		
$0112	$293E			GOTO	_EXIT2
;math.ppas,1593 :: 		
$0113	$1003			BCF	STATUS, C
;math.ppas,1602 :: 		
$0114	$	_LOOP:
;math.ppas,1603 :: 		
$0114	$0CFB			RRF	STACK_11, F
;math.ppas,1604 :: 		
$0115	$0CFA			RRF	STACK_10, F
;math.ppas,1605 :: 		
$0116	$0CF9			RRF	STACK_9, F
;math.ppas,1606 :: 		
$0117	$0CF8			RRF	STACK_8, F
;math.ppas,1607 :: 		
$0118	$0CF3			RRF	STACK_3, F
;math.ppas,1608 :: 		
$0119	$0CF2			RRF	STACK_2, F
;math.ppas,1609 :: 		
$011A	$0CF1			RRF	STACK_1, F
;math.ppas,1614 :: 		
$011B	$0CF0			RRF	STACK_0, F
;math.ppas,1615 :: 		
$011C	$1C03			BTFSS	STATUS, C
;math.ppas,1616 :: 		
$011D	$2910			GOTO	_NEXT
;math.ppas,1617 :: 		
$011E	$03FC			DECF	STACK_12, F
;math.ppas,1618 :: 		
$011F	$1903			BTFSC	STATUS, Z
;math.ppas,1625 :: 		
$0120	$2930			GOTO	_EXIT1
;math.ppas,1626 :: 		
$0121	$0874			MOVF	STACK_4, W
;math.ppas,1627 :: 		
$0122	$07F8			ADDWF	STACK_8, F
;math.ppas,1628 :: 		
$0123	$0875			MOVF	STACK_5, W
;math.ppas,1629 :: 		
$0124	$1803			BTFSC	STATUS, C
;math.ppas,1630 :: 		
$0125	$0F75			INCFSZ	STACK_5, W
;math.ppas,1631 :: 		
$0126	$07F9			ADDWF	STACK_9, F
;math.ppas,1632 :: 		
$0127	$0876			MOVF	STACK_6, W
;math.ppas,1633 :: 		
$0128	$1803			BTFSC	STATUS, C
;math.ppas,1634 :: 		
$0129	$0F76			INCFSZ	STACK_6, W
;math.ppas,1635 :: 		
$012A	$07FA			ADDWF	STACK_10, F
;math.ppas,1636 :: 		
$012B	$0877			MOVF	STACK_7, W
;math.ppas,1637 :: 		
$012C	$1803			BTFSC	STATUS, C
;math.ppas,1638 :: 		
$012D	$0F77			INCFSZ	STACK_7, W
;math.ppas,1640 :: 		
$012E	$07FB			ADDWF	STACK_11, F
;math.ppas,1642 :: 		
$012F	$2914			GOTO	_LOOP
;math.ppas,1647 :: 		
$0130	$	_EXIT1:
;math.ppas,1648 :: 		
$0130	$0874			MOVF	STACK_4, W
;math.ppas,1649 :: 		
$0131	$07F8			ADDWF	STACK_8, F
;math.ppas,1650 :: 		
$0132	$0875			MOVF	STACK_5, W
;math.ppas,1651 :: 		
$0133	$1803			BTFSC	STATUS, C
;math.ppas,1652 :: 		
$0134	$0F75			INCFSZ	STACK_5, W
;math.ppas,1653 :: 		
$0135	$07F9			ADDWF	STACK_9, F
;math.ppas,1654 :: 		
$0136	$0876			MOVF	STACK_6, W
;math.ppas,1655 :: 		
$0137	$1803			BTFSC	STATUS, C
;math.ppas,1656 :: 		
$0138	$0F76			INCFSZ	STACK_6, W
;math.ppas,1657 :: 		
$0139	$07FA			ADDWF	STACK_10, F
;math.ppas,1658 :: 		
$013A	$0877			MOVF	STACK_7, W
;math.ppas,1659 :: 		
$013B	$1803			BTFSC	STATUS, C
;math.ppas,1660 :: 		
$013C	$0F77			INCFSZ	STACK_7, W
;math.ppas,1663 :: 		
$013D	$07FB			ADDWF	STACK_11, F
;math.ppas,1664 :: 		
$013E	$	_EXIT2:
;math.ppas,1665 :: 		
$013E	$	math_L_13:
;math.ppas,1666 :: 		
$013E	$0008			RETURN
$0004	$	_vdelay_ms:
;delays.pbas,65 :: 		dim NumberOfCyc as dword
;delays.pbas,67 :: 		NumberOfCyc = Clock_kHz >> 2 ' this will be done by compiler, no asm will be genereated except that for assignment
$0004	$30E8			MOVLW	232
$0005	$1303			BCF	STATUS, RP1
$0006	$1283			BCF	STATUS, RP0
$0007	$00AB			MOVWF	_vdelay_ms_local_numberofcyc
$0008	$3003			MOVLW	3
$0009	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+1
$000A	$01AD			CLRF	_vdelay_ms_local_numberofcyc+2
$000B	$01AE			CLRF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,68 :: 		NumberOfCyc = Time_ms * NumberOfCyc
$000C	$0829			MOVF	FARG_VDelay_ms, 0
$000D	$00F0			MOVWF	STACK_0
$000E	$082A			MOVF	FARG_VDelay_ms+1, 0
$000F	$00F1			MOVWF	STACK_0+1
$0010	$01F2			CLRF	STACK_0+2
$0011	$01F3			CLRF	STACK_0+3
$0012	$30E8			MOVLW	232
$0013	$00F4			MOVWF	STACK_4
$0014	$3003			MOVLW	3
$0015	$00F5			MOVWF	STACK_4+1
$0016	$3000			MOVLW	0
$0017	$00F6			MOVWF	STACK_4+2
$0018	$3000			MOVLW	0
$0019	$00F7			MOVWF	STACK_4+3
$001A	$2108			CALL	_mul_32x32_u
$001B	$0870			MOVF	STACK_0, 0
$001C	$00AB			MOVWF	_vdelay_ms_local_numberofcyc
$001D	$0871			MOVF	STACK_0+1, 0
$001E	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+1
$001F	$0872			MOVF	STACK_0+2, 0
$0020	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+2
$0021	$0873			MOVF	STACK_0+3, 0
$0022	$00AE			MOVWF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,69 :: 		if NumberOfCyc < 675 then
$0023	$3000			MOVLW	0
$0024	$0273			SUBWF	STACK_0+3, 0
$0025	$1D03			BTFSS	STATUS, Z
$0026	$2831			GOTO	L_VDelay_ms_0
$0027	$3000			MOVLW	0
$0028	$0272			SUBWF	STACK_0+2, 0
$0029	$1D03			BTFSS	STATUS, Z
$002A	$2831			GOTO	L_VDelay_ms_0
$002B	$3002			MOVLW	2
$002C	$0271			SUBWF	STACK_0+1, 0
$002D	$1D03			BTFSS	STATUS, Z
$002E	$2831			GOTO	L_VDelay_ms_0
$002F	$30A3			MOVLW	163
$0030	$0270			SUBWF	STACK_0, 0
$0031	$	L_VDelay_ms_0:
$0031	$1803			BTFSC	STATUS, C
$0032	$2834			GOTO	delays_L_19
$0033	$	delays_L_18:
;delays.pbas,70 :: 		exit
$0033	$288B			GOTO	delays_L_17
$0034	$	delays_L_19:
;delays.pbas,71 :: 		end if
$0034	$	delays_L_20:
;delays.pbas,72 :: 		NumberOfCyc = NumberOfCyc - 675
$0034	$30A3			MOVLW	163
$0035	$00F0			MOVWF	STACK_0
$0036	$3002			MOVLW	2
$0037	$00F1			MOVWF	STACK_0+1
$0038	$01F2			CLRF	STACK_0+2
$0039	$01F3			CLRF	STACK_0+3
$003A	$082B			MOVF	_vdelay_ms_local_numberofcyc, 0
$003B	$00F4			MOVWF	STACK_4
$003C	$082C			MOVF	_vdelay_ms_local_numberofcyc+1, 0
$003D	$00F5			MOVWF	STACK_4+1
$003E	$082D			MOVF	_vdelay_ms_local_numberofcyc+2, 0
$003F	$00F6			MOVWF	STACK_4+2
$0040	$082E			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$0041	$00F7			MOVWF	STACK_4+3
$0042	$0870			MOVF	STACK_0, 0
$0043	$02F4			SUBWF	STACK_4, 1
$0044	$0871			MOVF	STACK_0+1, 0
$0045	$1C03			BTFSS	STATUS, C
$0046	$0F71			INCFSZ	STACK_0+1, 0
$0047	$02F5			SUBWF	STACK_4+1, 1
$0048	$0872			MOVF	STACK_0+2, 0
$0049	$1C03			BTFSS	STATUS, C
$004A	$0F72			INCFSZ	STACK_0+2, 0
$004B	$02F6			SUBWF	STACK_4+2, 1
$004C	$0873			MOVF	STACK_0+3, 0
$004D	$1C03			BTFSS	STATUS, C
$004E	$0F73			INCFSZ	STACK_0+3, 0
$004F	$02F7			SUBWF	STACK_4+3, 1
$0050	$0874			MOVF	STACK_4, 0
$0051	$00AB			MOVWF	_vdelay_ms_local_numberofcyc
$0052	$0875			MOVF	STACK_4+1, 0
$0053	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+1
$0054	$0876			MOVF	STACK_4+2, 0
$0055	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+2
$0056	$0877			MOVF	STACK_4+3, 0
$0057	$00AE			MOVWF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,73 :: 		NumberOfCyc = NumberOfCyc >> 5  ' Dec and While below take around 32 instructions
$0058	$3005			MOVLW	5
$0059	$00F0			MOVWF	STACK_0
$005A	$0874			MOVF	STACK_4, 0
$005B	$00AB			MOVWF	_vdelay_ms_local_numberofcyc
$005C	$0875			MOVF	STACK_4+1, 0
$005D	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+1
$005E	$0876			MOVF	STACK_4+2, 0
$005F	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+2
$0060	$0877			MOVF	STACK_4+3, 0
$0061	$00AE			MOVWF	_vdelay_ms_local_numberofcyc+3
$0062	$0870			MOVF	STACK_0, 0
$0063	$	L_VDelay_ms_1:
$0063	$1903			BTFSC	STATUS, Z
$0064	$286C			GOTO	L_VDelay_ms_2
$0065	$0CAE			RRF	_vdelay_ms_local_numberofcyc+3, 1
$0066	$0CAD			RRF	_vdelay_ms_local_numberofcyc+2, 1
$0067	$0CAC			RRF	_vdelay_ms_local_numberofcyc+1, 1
$0068	$0CAB			RRF	_vdelay_ms_local_numberofcyc, 1
$0069	$13AE			BCF	_vdelay_ms_local_numberofcyc+3, 7
$006A	$3FFF			ADDLW	255
$006B	$2863			GOTO	L_VDelay_ms_1
$006C	$	L_VDelay_ms_2:
;delays.pbas,74 :: 		while ( Lo(NumberOfCyc) or Hi(NumberOfCyc) or Higher(NumberOfCyc) or Highest(NumberOfCyc) ) <> 0
$006C	$	delays_L_22:
$006C	$30FF			MOVLW	255
$006D	$052B			ANDWF	_vdelay_ms_local_numberofcyc, 0
$006E	$00F8			MOVWF	STACK_8
$006F	$082C			MOVF	_vdelay_ms_local_numberofcyc+1, 0
$0070	$00F0			MOVWF	STACK_0
$0071	$0870			MOVF	STACK_0, 0
$0072	$04F8			IORWF	STACK_8, 1
$0073	$082D			MOVF	_vdelay_ms_local_numberofcyc+2, 0
$0074	$00F0			MOVWF	STACK_0
$0075	$082E			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$0076	$00F1			MOVWF	STACK_0+1
$0077	$0870			MOVF	STACK_0, 0
$0078	$04F8			IORWF	STACK_8, 1
$0079	$082E			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$007A	$00F0			MOVWF	STACK_0
$007B	$0870			MOVF	STACK_0, 0
$007C	$0478			IORWF	STACK_8, 0
$007D	$00F4			MOVWF	STACK_4
$007E	$0874			MOVF	STACK_4, 0
$007F	$3A00			XORLW	0
$0080	$1903			BTFSC	STATUS, Z
$0081	$288B			GOTO	delays_L_23
;delays.pbas,75 :: 		Dec(NumberOfCyc)
$0082	$3001			MOVLW	1
$0083	$02AB			SUBWF	_vdelay_ms_local_numberofcyc, 1
$0084	$1C03			BTFSS	STATUS, C
$0085	$02AC			SUBWF	_vdelay_ms_local_numberofcyc+1, 1
$0086	$1C03			BTFSS	STATUS, C
$0087	$02AD			SUBWF	_vdelay_ms_local_numberofcyc+2, 1
$0088	$1C03			BTFSS	STATUS, C
$0089	$02AE			SUBWF	_vdelay_ms_local_numberofcyc+3, 1
$008A	$286C			GOTO	delays_L_22
$008B	$	delays_L_23:
;delays.pbas,76 :: 		wend
$008B	$	delays_L_17:
$008B	$0008			RETURN
$008C	$	_giramotor:
;DriverDeControle.pbas,7 :: 		sub procedure giraMotor(dim invert    as boolean)
;DriverDeControle.pbas,9 :: 		while PORTA.0 = 1
$008C	$	DriverDeControle_L_2:
$008C	$3001			MOVLW	1
$008D	$1303			BCF	STATUS, RP1
$008E	$1283			BCF	STATUS, RP0
$008F	$0505			ANDWF	PORTA, 0
$0090	$00F1			MOVWF	STACK_1
$0091	$0871			MOVF	STACK_1, 0
$0092	$3A01			XORLW	1
$0093	$1D03			BTFSS	STATUS, Z
$0094	$28D6			GOTO	DriverDeControle_L_3
;DriverDeControle.pbas,10 :: 		if invert = false then
$0095	$0825			MOVF	FARG_giraMotor, 0
$0096	$3A00			XORLW	0
$0097	$1D03			BTFSS	STATUS, Z
$0098	$28B5			GOTO	DriverDeControle_L_7
$0099	$	DriverDeControle_L_6:
;DriverDeControle.pbas,11 :: 		PORTC=1 VDelay_ms(tmpOn)
$0099	$3001			MOVLW	1
$009A	$0087			MOVWF	PORTC
$009B	$0820			MOVF	_tmpon, 0
$009C	$00A9			MOVWF	FARG_VDelay_ms
$009D	$0821			MOVF	_tmpon+1, 0
$009E	$00AA			MOVWF	FARG_VDelay_ms+1
$009F	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,12 :: 		PORTC=2 vdelay_ms(tmpOn)
$00A0	$3002			MOVLW	2
$00A1	$0087			MOVWF	PORTC
$00A2	$0820			MOVF	_tmpon, 0
$00A3	$00A9			MOVWF	FARG_VDelay_ms
$00A4	$0821			MOVF	_tmpon+1, 0
$00A5	$00AA			MOVWF	FARG_VDelay_ms+1
$00A6	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,13 :: 		PORTC=4 vdelay_ms(tmpOn)
$00A7	$3004			MOVLW	4
$00A8	$0087			MOVWF	PORTC
$00A9	$0820			MOVF	_tmpon, 0
$00AA	$00A9			MOVWF	FARG_VDelay_ms
$00AB	$0821			MOVF	_tmpon+1, 0
$00AC	$00AA			MOVWF	FARG_VDelay_ms+1
$00AD	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,14 :: 		PORTC=8 vdelay_ms(tmpOn)
$00AE	$3008			MOVLW	8
$00AF	$0087			MOVWF	PORTC
$00B0	$0820			MOVF	_tmpon, 0
$00B1	$00A9			MOVWF	FARG_VDelay_ms
$00B2	$0821			MOVF	_tmpon+1, 0
$00B3	$00AA			MOVWF	FARG_VDelay_ms+1
$00B4	$2004			CALL	_vdelay_ms
$00B5	$	DriverDeControle_L_7:
;DriverDeControle.pbas,15 :: 		end if
$00B5	$	DriverDeControle_L_8:
;DriverDeControle.pbas,16 :: 		if invert = true then
$00B5	$0825			MOVF	FARG_giraMotor, 0
$00B6	$3AFF			XORLW	255
$00B7	$1D03			BTFSS	STATUS, Z
$00B8	$28D5			GOTO	DriverDeControle_L_10
$00B9	$	DriverDeControle_L_9:
;DriverDeControle.pbas,17 :: 		PORTC=8 vdelay_ms(tmpOn)
$00B9	$3008			MOVLW	8
$00BA	$0087			MOVWF	PORTC
$00BB	$0820			MOVF	_tmpon, 0
$00BC	$00A9			MOVWF	FARG_VDelay_ms
$00BD	$0821			MOVF	_tmpon+1, 0
$00BE	$00AA			MOVWF	FARG_VDelay_ms+1
$00BF	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,18 :: 		PORTC=4 vdelay_ms(tmpOn)
$00C0	$3004			MOVLW	4
$00C1	$0087			MOVWF	PORTC
$00C2	$0820			MOVF	_tmpon, 0
$00C3	$00A9			MOVWF	FARG_VDelay_ms
$00C4	$0821			MOVF	_tmpon+1, 0
$00C5	$00AA			MOVWF	FARG_VDelay_ms+1
$00C6	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,19 :: 		PORTC=2 vdelay_ms(tmpOn)
$00C7	$3002			MOVLW	2
$00C8	$0087			MOVWF	PORTC
$00C9	$0820			MOVF	_tmpon, 0
$00CA	$00A9			MOVWF	FARG_VDelay_ms
$00CB	$0821			MOVF	_tmpon+1, 0
$00CC	$00AA			MOVWF	FARG_VDelay_ms+1
$00CD	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,20 :: 		PORTC=1 vdelay_ms(tmpOn)
$00CE	$3001			MOVLW	1
$00CF	$0087			MOVWF	PORTC
$00D0	$0820			MOVF	_tmpon, 0
$00D1	$00A9			MOVWF	FARG_VDelay_ms
$00D2	$0821			MOVF	_tmpon+1, 0
$00D3	$00AA			MOVWF	FARG_VDelay_ms+1
$00D4	$2004			CALL	_vdelay_ms
$00D5	$	DriverDeControle_L_10:
;DriverDeControle.pbas,21 :: 		end if
$00D5	$	DriverDeControle_L_11:
$00D5	$288C			GOTO	DriverDeControle_L_2
$00D6	$	DriverDeControle_L_3:
;DriverDeControle.pbas,22 :: 		wend
$00D6	$	DriverDeControle_L_0:
$00D6	$0008			RETURN
$013F	$	_usart_init:
;USART_b75.ppas,22 :: 		
;USART_b75.ppas,24 :: 		
$013F	$	USART_b75_L_1:
$013F	$1303			BCF	STATUS, RP1
$0140	$1683			BSF	STATUS, RP0
$0141	$1698			BSF	TXSTA, 5
$0142	$	USART_b75_L_2:
;USART_b75.ppas,25 :: 		
$0142	$3090			MOVLW	144
$0143	$1283			BCF	STATUS, RP0
$0144	$0098			MOVWF	RCSTA
;USART_b75.ppas,31 :: 		
$0145	$	USART_b75_L_3:
$0145	$1683			BSF	STATUS, RP0
$0146	$1686			BSF	TRISB, 5
$0147	$	USART_b75_L_4:
;USART_b75.ppas,32 :: 		
$0147	$	USART_b75_L_5:
$0147	$1786			BSF	TRISB, 7
$0148	$	USART_b75_L_6:
;USART_b75.ppas,35 :: 		
$0148	$	USART_b75_L_8:
$0148	$1283			BCF	STATUS, RP0
$0149	$080C			MOVF	PIR1, 0
$014A	$00F1			MOVWF	STACK_1
$014B	$3000			MOVLW	0
$014C	$1AF1			BTFSC	STACK_1, 5
$014D	$3001			MOVLW	1
$014E	$00F1			MOVWF	STACK_1
$014F	$0871			MOVF	STACK_1, 0
$0150	$3A01			XORLW	1
$0151	$1903			BTFSC	STATUS, Z
$0152	$2948			GOTO	USART_b75_L_8
;USART_b75.ppas,36 :: 		
$0153	$	USART_b75_L_9:
$0153	$	USART_b75_L_0:
;USART_b75.ppas,37 :: 		
$0153	$0008			RETURN
$00D7	$	_main:
;DriverDeControle.pbas,28 :: 		main:
$00D7	$	_main_main:
;DriverDeControle.pbas,29 :: 		TRISA=%11111111
$00D7	$30FF			MOVLW	255
$00D8	$1303			BCF	STATUS, RP1
$00D9	$1683			BSF	STATUS, RP0
$00DA	$0085			MOVWF	TRISA
;DriverDeControle.pbas,30 :: 		TRISB=%11111111
$00DB	$30FF			MOVLW	255
$00DC	$0086			MOVWF	TRISB
;DriverDeControle.pbas,31 :: 		TRISC=%00000000
$00DD	$0187			CLRF	TRISC, 1
;DriverDeControle.pbas,32 :: 		INTCON=%00000000
$00DE	$018B			CLRF	INTCON, 1
;DriverDeControle.pbas,33 :: 		ANSEL=%00000000
$00DF	$1703			BSF	STATUS, RP1
$00E0	$1283			BCF	STATUS, RP0
$00E1	$019E			CLRF	ANSEL, 1
;DriverDeControle.pbas,34 :: 		ANSELH=%00000000
$00E2	$019F			CLRF	ANSELH, 1
;DriverDeControle.pbas,35 :: 		PORTA=0
$00E3	$1303			BCF	STATUS, RP1
$00E4	$0185			CLRF	PORTA, 1
;DriverDeControle.pbas,36 :: 		PORTC=0
$00E5	$0187			CLRF	PORTC, 1
;DriverDeControle.pbas,38 :: 		Usart_Init(9600)
$00E6	$3019			MOVLW	25
$00E7	$1683			BSF	STATUS, RP0
$00E8	$0099			MOVWF	SPBRG
$00E9	$1518			BSF	TXSTA, BRGH
$00EA	$213F			CALL	_usart_init
;DriverDeControle.pbas,40 :: 		tmpOn=50
$00EB	$3032			MOVLW	50
$00EC	$00A0			MOVWF	_tmpon
$00ED	$01A1			CLRF	_tmpon+1
;DriverDeControle.pbas,41 :: 		tmpOff=0
$00EE	$01A2			CLRF	_tmpoff
$00EF	$01A3			CLRF	_tmpoff+1
;DriverDeControle.pbas,42 :: 		invert=false
$00F0	$01A4			CLRF	_invert, 1
;DriverDeControle.pbas,44 :: 		executa:
$00F1	$	_main_executa:
;DriverDeControle.pbas,46 :: 		if PORTA.1 = 1 then
$00F1	$3000			MOVLW	0
$00F2	$1885			BTFSC	PORTA, 1
$00F3	$3001			MOVLW	1
$00F4	$00F1			MOVWF	STACK_1
$00F5	$0871			MOVF	STACK_1, 0
$00F6	$3A01			XORLW	1
$00F7	$1D03			BTFSS	STATUS, Z
$00F8	$28FC			GOTO	DriverDeControle_L_14
$00F9	$	DriverDeControle_L_13:
;DriverDeControle.pbas,47 :: 		giraMotor(true)
$00F9	$30FF			MOVLW	255
$00FA	$00A5			MOVWF	FARG_giraMotor
$00FB	$208C			CALL	_giramotor
$00FC	$	DriverDeControle_L_14:
;DriverDeControle.pbas,48 :: 		end if
$00FC	$	DriverDeControle_L_15:
;DriverDeControle.pbas,49 :: 		if PORTA.1 = 0 then
$00FC	$3000			MOVLW	0
$00FD	$1885			BTFSC	PORTA, 1
$00FE	$3001			MOVLW	1
$00FF	$00F1			MOVWF	STACK_1
$0100	$0871			MOVF	STACK_1, 0
$0101	$3A00			XORLW	0
$0102	$1D03			BTFSS	STATUS, Z
$0103	$2906			GOTO	DriverDeControle_L_17
$0104	$	DriverDeControle_L_16:
;DriverDeControle.pbas,50 :: 		giraMotor(false)
$0104	$01A5			CLRF	FARG_giraMotor, 1
$0105	$208C			CALL	_giramotor
$0106	$	DriverDeControle_L_17:
;DriverDeControle.pbas,51 :: 		end if
$0106	$	DriverDeControle_L_18:
;DriverDeControle.pbas,53 :: 		goto executa
$0106	$28F1			GOTO	_main_executa
$0107	$2907			GOTO	$
