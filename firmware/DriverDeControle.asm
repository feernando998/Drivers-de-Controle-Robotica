;  Assembly code generated by mikroVirtualMachine - V. 5.0.0.3
;  Date/Time: 09/09/2020 21:43:04
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$28D7			GOTO	_main
$0107	$	_mul_32x32_u:
$0107	$1303			BCF	STATUS, RP1
$0108	$1283			BCF	STATUS, RP0
;math.ppas,1570 :: 		
;math.ppas,1578 :: 		
$0109	$3022			MOVLW	34
;math.ppas,1579 :: 		
$010A	$00FC			MOVWF	STACK_12
;math.ppas,1580 :: 		
$010B	$01F8			CLRF	STACK_8
;math.ppas,1581 :: 		
$010C	$01F9			CLRF	STACK_9
;math.ppas,1582 :: 		
$010D	$01FA			CLRF	STACK_10
;math.ppas,1584 :: 		
$010E	$01FB			CLRF	STACK_11
;math.ppas,1588 :: 		
$010F	$	_NEXT:
;math.ppas,1589 :: 		
$010F	$03FC			DECF	STACK_12, F
;math.ppas,1590 :: 		
$0110	$1903			BTFSC	STATUS, Z
;math.ppas,1591 :: 		
$0111	$293D			GOTO	_EXIT2
;math.ppas,1593 :: 		
$0112	$1003			BCF	STATUS, C
;math.ppas,1602 :: 		
$0113	$	_LOOP:
;math.ppas,1603 :: 		
$0113	$0CFB			RRF	STACK_11, F
;math.ppas,1604 :: 		
$0114	$0CFA			RRF	STACK_10, F
;math.ppas,1605 :: 		
$0115	$0CF9			RRF	STACK_9, F
;math.ppas,1606 :: 		
$0116	$0CF8			RRF	STACK_8, F
;math.ppas,1607 :: 		
$0117	$0CF3			RRF	STACK_3, F
;math.ppas,1608 :: 		
$0118	$0CF2			RRF	STACK_2, F
;math.ppas,1609 :: 		
$0119	$0CF1			RRF	STACK_1, F
;math.ppas,1614 :: 		
$011A	$0CF0			RRF	STACK_0, F
;math.ppas,1615 :: 		
$011B	$1C03			BTFSS	STATUS, C
;math.ppas,1616 :: 		
$011C	$290F			GOTO	_NEXT
;math.ppas,1617 :: 		
$011D	$03FC			DECF	STACK_12, F
;math.ppas,1618 :: 		
$011E	$1903			BTFSC	STATUS, Z
;math.ppas,1625 :: 		
$011F	$292F			GOTO	_EXIT1
;math.ppas,1626 :: 		
$0120	$0874			MOVF	STACK_4, W
;math.ppas,1627 :: 		
$0121	$07F8			ADDWF	STACK_8, F
;math.ppas,1628 :: 		
$0122	$0875			MOVF	STACK_5, W
;math.ppas,1629 :: 		
$0123	$1803			BTFSC	STATUS, C
;math.ppas,1630 :: 		
$0124	$0F75			INCFSZ	STACK_5, W
;math.ppas,1631 :: 		
$0125	$07F9			ADDWF	STACK_9, F
;math.ppas,1632 :: 		
$0126	$0876			MOVF	STACK_6, W
;math.ppas,1633 :: 		
$0127	$1803			BTFSC	STATUS, C
;math.ppas,1634 :: 		
$0128	$0F76			INCFSZ	STACK_6, W
;math.ppas,1635 :: 		
$0129	$07FA			ADDWF	STACK_10, F
;math.ppas,1636 :: 		
$012A	$0877			MOVF	STACK_7, W
;math.ppas,1637 :: 		
$012B	$1803			BTFSC	STATUS, C
;math.ppas,1638 :: 		
$012C	$0F77			INCFSZ	STACK_7, W
;math.ppas,1640 :: 		
$012D	$07FB			ADDWF	STACK_11, F
;math.ppas,1642 :: 		
$012E	$2913			GOTO	_LOOP
;math.ppas,1647 :: 		
$012F	$	_EXIT1:
;math.ppas,1648 :: 		
$012F	$0874			MOVF	STACK_4, W
;math.ppas,1649 :: 		
$0130	$07F8			ADDWF	STACK_8, F
;math.ppas,1650 :: 		
$0131	$0875			MOVF	STACK_5, W
;math.ppas,1651 :: 		
$0132	$1803			BTFSC	STATUS, C
;math.ppas,1652 :: 		
$0133	$0F75			INCFSZ	STACK_5, W
;math.ppas,1653 :: 		
$0134	$07F9			ADDWF	STACK_9, F
;math.ppas,1654 :: 		
$0135	$0876			MOVF	STACK_6, W
;math.ppas,1655 :: 		
$0136	$1803			BTFSC	STATUS, C
;math.ppas,1656 :: 		
$0137	$0F76			INCFSZ	STACK_6, W
;math.ppas,1657 :: 		
$0138	$07FA			ADDWF	STACK_10, F
;math.ppas,1658 :: 		
$0139	$0877			MOVF	STACK_7, W
;math.ppas,1659 :: 		
$013A	$1803			BTFSC	STATUS, C
;math.ppas,1660 :: 		
$013B	$0F77			INCFSZ	STACK_7, W
;math.ppas,1663 :: 		
$013C	$07FB			ADDWF	STACK_11, F
;math.ppas,1664 :: 		
$013D	$	_EXIT2:
;math.ppas,1665 :: 		
$013D	$	math_L_13:
;math.ppas,1666 :: 		
$013D	$0008			RETURN
$0004	$	_vdelay_ms:
;delays.pbas,65 :: 		dim NumberOfCyc as dword
;delays.pbas,67 :: 		NumberOfCyc = Clock_kHz >> 2 ' this will be done by compiler, no asm will be genereated except that for assignment
$0004	$30E8			MOVLW	232
$0005	$1303			BCF	STATUS, RP1
$0006	$1283			BCF	STATUS, RP0
$0007	$00AA			MOVWF	_vdelay_ms_local_numberofcyc
$0008	$3003			MOVLW	3
$0009	$00AB			MOVWF	_vdelay_ms_local_numberofcyc+1
$000A	$01AC			CLRF	_vdelay_ms_local_numberofcyc+2
$000B	$01AD			CLRF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,68 :: 		NumberOfCyc = Time_ms * NumberOfCyc
$000C	$0828			MOVF	FARG_VDelay_ms, 0
$000D	$00F0			MOVWF	STACK_0
$000E	$0829			MOVF	FARG_VDelay_ms+1, 0
$000F	$00F1			MOVWF	STACK_0+1
$0010	$01F2			CLRF	STACK_0+2
$0011	$01F3			CLRF	STACK_0+3
$0012	$30E8			MOVLW	232
$0013	$00F4			MOVWF	STACK_4
$0014	$3003			MOVLW	3
$0015	$00F5			MOVWF	STACK_4+1
$0016	$3000			MOVLW	0
$0017	$00F6			MOVWF	STACK_4+2
$0018	$3000			MOVLW	0
$0019	$00F7			MOVWF	STACK_4+3
$001A	$2107			CALL	_mul_32x32_u
$001B	$0870			MOVF	STACK_0, 0
$001C	$00AA			MOVWF	_vdelay_ms_local_numberofcyc
$001D	$0871			MOVF	STACK_0+1, 0
$001E	$00AB			MOVWF	_vdelay_ms_local_numberofcyc+1
$001F	$0872			MOVF	STACK_0+2, 0
$0020	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+2
$0021	$0873			MOVF	STACK_0+3, 0
$0022	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,69 :: 		if NumberOfCyc < 675 then
$0023	$3000			MOVLW	0
$0024	$0273			SUBWF	STACK_0+3, 0
$0025	$1D03			BTFSS	STATUS, Z
$0026	$2831			GOTO	L_VDelay_ms_0
$0027	$3000			MOVLW	0
$0028	$0272			SUBWF	STACK_0+2, 0
$0029	$1D03			BTFSS	STATUS, Z
$002A	$2831			GOTO	L_VDelay_ms_0
$002B	$3002			MOVLW	2
$002C	$0271			SUBWF	STACK_0+1, 0
$002D	$1D03			BTFSS	STATUS, Z
$002E	$2831			GOTO	L_VDelay_ms_0
$002F	$30A3			MOVLW	163
$0030	$0270			SUBWF	STACK_0, 0
$0031	$	L_VDelay_ms_0:
$0031	$1803			BTFSC	STATUS, C
$0032	$2834			GOTO	delays_L_19
$0033	$	delays_L_18:
;delays.pbas,70 :: 		exit
$0033	$288B			GOTO	delays_L_17
$0034	$	delays_L_19:
;delays.pbas,71 :: 		end if
$0034	$	delays_L_20:
;delays.pbas,72 :: 		NumberOfCyc = NumberOfCyc - 675
$0034	$30A3			MOVLW	163
$0035	$00F0			MOVWF	STACK_0
$0036	$3002			MOVLW	2
$0037	$00F1			MOVWF	STACK_0+1
$0038	$01F2			CLRF	STACK_0+2
$0039	$01F3			CLRF	STACK_0+3
$003A	$082A			MOVF	_vdelay_ms_local_numberofcyc, 0
$003B	$00F4			MOVWF	STACK_4
$003C	$082B			MOVF	_vdelay_ms_local_numberofcyc+1, 0
$003D	$00F5			MOVWF	STACK_4+1
$003E	$082C			MOVF	_vdelay_ms_local_numberofcyc+2, 0
$003F	$00F6			MOVWF	STACK_4+2
$0040	$082D			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$0041	$00F7			MOVWF	STACK_4+3
$0042	$0870			MOVF	STACK_0, 0
$0043	$02F4			SUBWF	STACK_4, 1
$0044	$0871			MOVF	STACK_0+1, 0
$0045	$1C03			BTFSS	STATUS, C
$0046	$0F71			INCFSZ	STACK_0+1, 0
$0047	$02F5			SUBWF	STACK_4+1, 1
$0048	$0872			MOVF	STACK_0+2, 0
$0049	$1C03			BTFSS	STATUS, C
$004A	$0F72			INCFSZ	STACK_0+2, 0
$004B	$02F6			SUBWF	STACK_4+2, 1
$004C	$0873			MOVF	STACK_0+3, 0
$004D	$1C03			BTFSS	STATUS, C
$004E	$0F73			INCFSZ	STACK_0+3, 0
$004F	$02F7			SUBWF	STACK_4+3, 1
$0050	$0874			MOVF	STACK_4, 0
$0051	$00AA			MOVWF	_vdelay_ms_local_numberofcyc
$0052	$0875			MOVF	STACK_4+1, 0
$0053	$00AB			MOVWF	_vdelay_ms_local_numberofcyc+1
$0054	$0876			MOVF	STACK_4+2, 0
$0055	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+2
$0056	$0877			MOVF	STACK_4+3, 0
$0057	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+3
;delays.pbas,73 :: 		NumberOfCyc = NumberOfCyc >> 5  ' Dec and While below take around 32 instructions
$0058	$3005			MOVLW	5
$0059	$00F0			MOVWF	STACK_0
$005A	$0874			MOVF	STACK_4, 0
$005B	$00AA			MOVWF	_vdelay_ms_local_numberofcyc
$005C	$0875			MOVF	STACK_4+1, 0
$005D	$00AB			MOVWF	_vdelay_ms_local_numberofcyc+1
$005E	$0876			MOVF	STACK_4+2, 0
$005F	$00AC			MOVWF	_vdelay_ms_local_numberofcyc+2
$0060	$0877			MOVF	STACK_4+3, 0
$0061	$00AD			MOVWF	_vdelay_ms_local_numberofcyc+3
$0062	$0870			MOVF	STACK_0, 0
$0063	$	L_VDelay_ms_1:
$0063	$1903			BTFSC	STATUS, Z
$0064	$286C			GOTO	L_VDelay_ms_2
$0065	$0CAD			RRF	_vdelay_ms_local_numberofcyc+3, 1
$0066	$0CAC			RRF	_vdelay_ms_local_numberofcyc+2, 1
$0067	$0CAB			RRF	_vdelay_ms_local_numberofcyc+1, 1
$0068	$0CAA			RRF	_vdelay_ms_local_numberofcyc, 1
$0069	$13AD			BCF	_vdelay_ms_local_numberofcyc+3, 7
$006A	$3FFF			ADDLW	255
$006B	$2863			GOTO	L_VDelay_ms_1
$006C	$	L_VDelay_ms_2:
;delays.pbas,74 :: 		while ( Lo(NumberOfCyc) or Hi(NumberOfCyc) or Higher(NumberOfCyc) or Highest(NumberOfCyc) ) <> 0
$006C	$	delays_L_22:
$006C	$30FF			MOVLW	255
$006D	$052A			ANDWF	_vdelay_ms_local_numberofcyc, 0
$006E	$00F8			MOVWF	STACK_8
$006F	$082B			MOVF	_vdelay_ms_local_numberofcyc+1, 0
$0070	$00F0			MOVWF	STACK_0
$0071	$0870			MOVF	STACK_0, 0
$0072	$04F8			IORWF	STACK_8, 1
$0073	$082C			MOVF	_vdelay_ms_local_numberofcyc+2, 0
$0074	$00F0			MOVWF	STACK_0
$0075	$082D			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$0076	$00F1			MOVWF	STACK_0+1
$0077	$0870			MOVF	STACK_0, 0
$0078	$04F8			IORWF	STACK_8, 1
$0079	$082D			MOVF	_vdelay_ms_local_numberofcyc+3, 0
$007A	$00F0			MOVWF	STACK_0
$007B	$0870			MOVF	STACK_0, 0
$007C	$0478			IORWF	STACK_8, 0
$007D	$00F4			MOVWF	STACK_4
$007E	$0874			MOVF	STACK_4, 0
$007F	$3A00			XORLW	0
$0080	$1903			BTFSC	STATUS, Z
$0081	$288B			GOTO	delays_L_23
;delays.pbas,75 :: 		Dec(NumberOfCyc)
$0082	$3001			MOVLW	1
$0083	$02AA			SUBWF	_vdelay_ms_local_numberofcyc, 1
$0084	$1C03			BTFSS	STATUS, C
$0085	$02AB			SUBWF	_vdelay_ms_local_numberofcyc+1, 1
$0086	$1C03			BTFSS	STATUS, C
$0087	$02AC			SUBWF	_vdelay_ms_local_numberofcyc+2, 1
$0088	$1C03			BTFSS	STATUS, C
$0089	$02AD			SUBWF	_vdelay_ms_local_numberofcyc+3, 1
$008A	$286C			GOTO	delays_L_22
$008B	$	delays_L_23:
;delays.pbas,76 :: 		wend
$008B	$	delays_L_17:
$008B	$0008			RETURN
$008C	$	_giramotor:
;DriverDeControle.pbas,6 :: 		sub procedure giraMotor(dim invert    as boolean)
;DriverDeControle.pbas,8 :: 		while PORTA.0 = 1
$008C	$	DriverDeControle_L_2:
$008C	$3001			MOVLW	1
$008D	$1303			BCF	STATUS, RP1
$008E	$1283			BCF	STATUS, RP0
$008F	$0505			ANDWF	PORTA, 0
$0090	$00F1			MOVWF	STACK_1
$0091	$0871			MOVF	STACK_1, 0
$0092	$3A01			XORLW	1
$0093	$1D03			BTFSS	STATUS, Z
$0094	$28D6			GOTO	DriverDeControle_L_3
;DriverDeControle.pbas,9 :: 		if invert = false then
$0095	$0824			MOVF	FARG_giraMotor, 0
$0096	$3A00			XORLW	0
$0097	$1D03			BTFSS	STATUS, Z
$0098	$28B5			GOTO	DriverDeControle_L_7
$0099	$	DriverDeControle_L_6:
;DriverDeControle.pbas,10 :: 		PORTC=1 VDelay_ms(tmpOn)
$0099	$3001			MOVLW	1
$009A	$0087			MOVWF	PORTC
$009B	$0820			MOVF	_tmpon, 0
$009C	$00A8			MOVWF	FARG_VDelay_ms
$009D	$0821			MOVF	_tmpon+1, 0
$009E	$00A9			MOVWF	FARG_VDelay_ms+1
$009F	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,11 :: 		PORTC=2 vdelay_ms(tmpOn)
$00A0	$3002			MOVLW	2
$00A1	$0087			MOVWF	PORTC
$00A2	$0820			MOVF	_tmpon, 0
$00A3	$00A8			MOVWF	FARG_VDelay_ms
$00A4	$0821			MOVF	_tmpon+1, 0
$00A5	$00A9			MOVWF	FARG_VDelay_ms+1
$00A6	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,12 :: 		PORTC=4 vdelay_ms(tmpOn)
$00A7	$3004			MOVLW	4
$00A8	$0087			MOVWF	PORTC
$00A9	$0820			MOVF	_tmpon, 0
$00AA	$00A8			MOVWF	FARG_VDelay_ms
$00AB	$0821			MOVF	_tmpon+1, 0
$00AC	$00A9			MOVWF	FARG_VDelay_ms+1
$00AD	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,13 :: 		PORTC=8 vdelay_ms(tmpOn)
$00AE	$3008			MOVLW	8
$00AF	$0087			MOVWF	PORTC
$00B0	$0820			MOVF	_tmpon, 0
$00B1	$00A8			MOVWF	FARG_VDelay_ms
$00B2	$0821			MOVF	_tmpon+1, 0
$00B3	$00A9			MOVWF	FARG_VDelay_ms+1
$00B4	$2004			CALL	_vdelay_ms
$00B5	$	DriverDeControle_L_7:
;DriverDeControle.pbas,14 :: 		end if
$00B5	$	DriverDeControle_L_8:
;DriverDeControle.pbas,15 :: 		if invert = true then
$00B5	$0824			MOVF	FARG_giraMotor, 0
$00B6	$3AFF			XORLW	255
$00B7	$1D03			BTFSS	STATUS, Z
$00B8	$28D5			GOTO	DriverDeControle_L_10
$00B9	$	DriverDeControle_L_9:
;DriverDeControle.pbas,16 :: 		PORTC=8 vdelay_ms(tmpOn)
$00B9	$3008			MOVLW	8
$00BA	$0087			MOVWF	PORTC
$00BB	$0820			MOVF	_tmpon, 0
$00BC	$00A8			MOVWF	FARG_VDelay_ms
$00BD	$0821			MOVF	_tmpon+1, 0
$00BE	$00A9			MOVWF	FARG_VDelay_ms+1
$00BF	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,17 :: 		PORTC=4 vdelay_ms(tmpOn)
$00C0	$3004			MOVLW	4
$00C1	$0087			MOVWF	PORTC
$00C2	$0820			MOVF	_tmpon, 0
$00C3	$00A8			MOVWF	FARG_VDelay_ms
$00C4	$0821			MOVF	_tmpon+1, 0
$00C5	$00A9			MOVWF	FARG_VDelay_ms+1
$00C6	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,18 :: 		PORTC=2 vdelay_ms(tmpOn)
$00C7	$3002			MOVLW	2
$00C8	$0087			MOVWF	PORTC
$00C9	$0820			MOVF	_tmpon, 0
$00CA	$00A8			MOVWF	FARG_VDelay_ms
$00CB	$0821			MOVF	_tmpon+1, 0
$00CC	$00A9			MOVWF	FARG_VDelay_ms+1
$00CD	$2004			CALL	_vdelay_ms
;DriverDeControle.pbas,19 :: 		PORTC=1 vdelay_ms(tmpOn)
$00CE	$3001			MOVLW	1
$00CF	$0087			MOVWF	PORTC
$00D0	$0820			MOVF	_tmpon, 0
$00D1	$00A8			MOVWF	FARG_VDelay_ms
$00D2	$0821			MOVF	_tmpon+1, 0
$00D3	$00A9			MOVWF	FARG_VDelay_ms+1
$00D4	$2004			CALL	_vdelay_ms
$00D5	$	DriverDeControle_L_10:
;DriverDeControle.pbas,20 :: 		end if
$00D5	$	DriverDeControle_L_11:
$00D5	$288C			GOTO	DriverDeControle_L_2
$00D6	$	DriverDeControle_L_3:
;DriverDeControle.pbas,21 :: 		wend
$00D6	$	DriverDeControle_L_0:
$00D6	$0008			RETURN
$013E	$	_usart_init:
;USART_b75.ppas,22 :: 		
;USART_b75.ppas,24 :: 		
$013E	$	USART_b75_L_1:
$013E	$1303			BCF	STATUS, RP1
$013F	$1683			BSF	STATUS, RP0
$0140	$1698			BSF	TXSTA, 5
$0141	$	USART_b75_L_2:
;USART_b75.ppas,25 :: 		
$0141	$3090			MOVLW	144
$0142	$1283			BCF	STATUS, RP0
$0143	$0098			MOVWF	RCSTA
;USART_b75.ppas,31 :: 		
$0144	$	USART_b75_L_3:
$0144	$1683			BSF	STATUS, RP0
$0145	$1686			BSF	TRISB, 5
$0146	$	USART_b75_L_4:
;USART_b75.ppas,32 :: 		
$0146	$	USART_b75_L_5:
$0146	$1786			BSF	TRISB, 7
$0147	$	USART_b75_L_6:
;USART_b75.ppas,35 :: 		
$0147	$	USART_b75_L_8:
$0147	$1283			BCF	STATUS, RP0
$0148	$080C			MOVF	PIR1, 0
$0149	$00F1			MOVWF	STACK_1
$014A	$3000			MOVLW	0
$014B	$1AF1			BTFSC	STACK_1, 5
$014C	$3001			MOVLW	1
$014D	$00F1			MOVWF	STACK_1
$014E	$0871			MOVF	STACK_1, 0
$014F	$3A01			XORLW	1
$0150	$1903			BTFSC	STATUS, Z
$0151	$2947			GOTO	USART_b75_L_8
;USART_b75.ppas,36 :: 		
$0152	$	USART_b75_L_9:
$0152	$	USART_b75_L_0:
;USART_b75.ppas,37 :: 		
$0152	$0008			RETURN
$00D7	$	_main:
;DriverDeControle.pbas,27 :: 		main:
$00D7	$	_main_main:
;DriverDeControle.pbas,28 :: 		TRISA=%11111111
$00D7	$30FF			MOVLW	255
$00D8	$1303			BCF	STATUS, RP1
$00D9	$1683			BSF	STATUS, RP0
$00DA	$0085			MOVWF	TRISA
;DriverDeControle.pbas,29 :: 		TRISB=%11111111
$00DB	$30FF			MOVLW	255
$00DC	$0086			MOVWF	TRISB
;DriverDeControle.pbas,30 :: 		TRISC=%00000000
$00DD	$0187			CLRF	TRISC, 1
;DriverDeControle.pbas,31 :: 		INTCON=%00000000
$00DE	$018B			CLRF	INTCON, 1
;DriverDeControle.pbas,32 :: 		ANSEL=%00000000
$00DF	$1703			BSF	STATUS, RP1
$00E0	$1283			BCF	STATUS, RP0
$00E1	$019E			CLRF	ANSEL, 1
;DriverDeControle.pbas,33 :: 		ANSELH=%00000000
$00E2	$019F			CLRF	ANSELH, 1
;DriverDeControle.pbas,34 :: 		PORTA=0
$00E3	$1303			BCF	STATUS, RP1
$00E4	$0185			CLRF	PORTA, 1
;DriverDeControle.pbas,35 :: 		PORTC=0
$00E5	$0187			CLRF	PORTC, 1
;DriverDeControle.pbas,37 :: 		Usart_Init(9600)
$00E6	$3019			MOVLW	25
$00E7	$1683			BSF	STATUS, RP0
$00E8	$0099			MOVWF	SPBRG
$00E9	$1518			BSF	TXSTA, BRGH
$00EA	$213E			CALL	_usart_init
;DriverDeControle.pbas,39 :: 		tmpOn=50
$00EB	$3032			MOVLW	50
$00EC	$00A0			MOVWF	_tmpon
$00ED	$01A1			CLRF	_tmpon+1
;DriverDeControle.pbas,40 :: 		tmpOff=0
$00EE	$01A2			CLRF	_tmpoff
$00EF	$01A3			CLRF	_tmpoff+1
;DriverDeControle.pbas,42 :: 		executa:
$00F0	$	_main_executa:
;DriverDeControle.pbas,44 :: 		if PORTA.1 = 1 then
$00F0	$3000			MOVLW	0
$00F1	$1885			BTFSC	PORTA, 1
$00F2	$3001			MOVLW	1
$00F3	$00F1			MOVWF	STACK_1
$00F4	$0871			MOVF	STACK_1, 0
$00F5	$3A01			XORLW	1
$00F6	$1D03			BTFSS	STATUS, Z
$00F7	$28FB			GOTO	DriverDeControle_L_14
$00F8	$	DriverDeControle_L_13:
;DriverDeControle.pbas,45 :: 		giraMotor(true)
$00F8	$30FF			MOVLW	255
$00F9	$00A4			MOVWF	FARG_giraMotor
$00FA	$208C			CALL	_giramotor
$00FB	$	DriverDeControle_L_14:
;DriverDeControle.pbas,46 :: 		end if
$00FB	$	DriverDeControle_L_15:
;DriverDeControle.pbas,47 :: 		if PORTA.1 = 0 then
$00FB	$3000			MOVLW	0
$00FC	$1885			BTFSC	PORTA, 1
$00FD	$3001			MOVLW	1
$00FE	$00F1			MOVWF	STACK_1
$00FF	$0871			MOVF	STACK_1, 0
$0100	$3A00			XORLW	0
$0101	$1D03			BTFSS	STATUS, Z
$0102	$2905			GOTO	DriverDeControle_L_17
$0103	$	DriverDeControle_L_16:
;DriverDeControle.pbas,48 :: 		giraMotor(false)
$0103	$01A4			CLRF	FARG_giraMotor, 1
$0104	$208C			CALL	_giramotor
$0105	$	DriverDeControle_L_17:
;DriverDeControle.pbas,49 :: 		end if
$0105	$	DriverDeControle_L_18:
;DriverDeControle.pbas,51 :: 		goto executa
$0105	$28F0			GOTO	_main_executa
$0106	$2906			GOTO	$
